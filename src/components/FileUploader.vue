...
<div>
  ...
  <v-progress-circular v-if="uploading && !uploadEnd" :size="100" :width="15" :rotate="360" :value="progressUpload" color="primary">
    {{ progressUpload }}%
  </v-progress-circular>
  <img
        v-if="uploadEnd"
...
//Thanks https://lovemewithoutall.github.io/it/vue-image-upload-to-firestorage/
...
export default {
  ...
  methods: {
    ...
    upload(file) {
      ...
      this.uploading = true
      this.uploadTask = db.ref('images/' + file.name).put(file)
    },
    deleteImage() {
      if (this.oldImgUrl === '') {
